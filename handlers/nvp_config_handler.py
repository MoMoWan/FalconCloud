...
class NvpConfigHandler(tornado.web.RequestHandler):
    """
    ...
    def post(self):
        ...
        nvps = jsondata['nvps']
        user_name = jsondata.get('username',"jawa")
        password = jsondata.get('password', "QAZ2wsx")

        # we will use the task id to query task status
        if ping (str(printer)):
            task = nvp_sets.delay(printer, nvps, user_name, password)
            # we will use the task id to query task status
            response = {
                'state': task.state,
                'id': task.id
