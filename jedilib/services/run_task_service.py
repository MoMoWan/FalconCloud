"""
This interfaces with RunTask Service. It will provide easy to use methods 
to use issue commands
"""
...
# =============================================================================
from .soap import RunTaskSoapMessage
...
class RunTaskService(object):
    ...
    def __init__(self, ip_address):
        """
        create Run Task Service for ip_address

        Args:
        ip_address (str): The ip address of unit
        """
    ...
    def run_task(self, task_name, method_identifier, parameters = None):
        ...
        """
        response = None
        msg = RunTaskSoapMessage(task_name, method_identifier, parameters, self.ip_address, RunTaskService.CURRENT_SERVICE).tostring()
        log.debug(msg)
        try:
            ...
            if "cannot be processed at the receiver" in str(ex):
                service = RunTaskService.NEXT_SERVICE[RunTaskService.CURRENT_SERVICE]
                log.debug("service is {}".format(service)
                )
                msg = RunTaskSoapMessage(task_name, method_identifier, parameters, self.ip_address, service).tostring()
                response = self.soap_client(msg)                
                log.debug("Switching service endpoint from {} to {}".format(RunTaskService.CURRENT_SERVICE, service))
                RunTaskService.CURRENT_SERVICE = service
        temp = BeautifulSoup(html.unescape(response), "xml")
        return temp.text
    ...
    def get_falcon_jedi_state(self, guid="D7BFA98C-6389-4661-8ED2-FC3775A800E0"):
        ...
        """
        value = self.get_spi_nvp_value(guid, "FalconJediState")
        return binascii.unhexlify(value).decode()
    ...
    def get_spi_nvp_batch(self, nvps):
        ...
        for nvp in nvps:
            reqCreator.add_get_element(nvp.Guid, nvp.Name)
        xmlReqData = reqCreator.generate_xml()
    ...
    def snmp_change_oid_access_permission(self, oid, access):
        ...
        :example:
        rt = RemoteTaskProxy(dut.RunTaskService)
    ...
    def set_spi_nvp_batch(self, nvps):
        ...
        reqCreator = RisNVPXmlRequestCreator()
        for nvp in nvps:
            reqCreator.add_set_element(nvp.Guid, nvp.Name, nvp.HexValue)
        xmlReqData = reqCreator.generate_xml()
